...
pre.CRS.test  <- function(c.beta,G,lambda=0,alpha=0.05,nj=1){
    ...
    #------------------------------------------------------------------
        q  = length(c.beta);   # # Number of clusters/estimators
        M  = dim(G)[2];        # # of elements in G
        if (length(nj)!=1 & length(nj)!=q){nj=1}

        Sn = sqrt(q)*sqrt(nj)*(c.beta-lambda);

        ObsT = abs(mean(Sn))/sd(Sn); # observed Test stat
        NewX = G*as.vector(Sn);                        # transformed data
...
CRS.CI<-function(c.beta,G,alpha=0.05,nj=1){
    ...
    lo.test<- CRS.test(c.beta,G,L,alpha); ite=1;
    ...
    hi.test<- CRS.test(c.beta,G,U,alpha); ite=1;
